...
mcl_handle* RunKernel1(char* kernel_path, uint32_t* frontier, uint32_t numVerts,
    ...
    err = mcl_task_set_arg(
        ...
        MCL_ARG_BUFFER | MCL_ARG_INPUT | MCL_ARG_DYNAMIC | MCL_ARG_RESIDENT
    );
    ...
    err = mcl_task_set_arg(
        ...
        MCL_ARG_BUFFER | MCL_ARG_INPUT | MCL_ARG_RESIDENT
    );
    MCL_CHECK_ERROR(err);
...
void RunTest1(ResultDatabase& resultDB, OptionParser& op, Graph *G)
{
    ...
    mcl_transfer** transfers = new mcl_transfer*[num_tasks];
    ...
    for (int j=0;j<passes;j++) {
        ...
        while (finished < num_tasks) {
            if(completed[i] != 1 && mcl_test(tasks[i]) == MCL_REQ_COMPLETED) {
                ...
                if(flag[i] == 1) {
                    ...
                    finished++;
                    completed[i] = 1;
                    transfers[i] = mcl_transfer_create(1, 1);
                    err = mcl_transfer_set_arg(transfers[i], 0, (void*)frontier[i], sizeof(uint32_t) * numVerts, 
                        MCL_ARG_RESIDENT | MCL_ARG_DYNAMIC | MCL_ARG_DONE | MCL_ARG_OUTPUT);
                    MCL_CHECK_ERROR(err);
                    err = mcl_transfer_exec(transfers[i], MCL_TASK_ANY);
                    MCL_CHECK_ERROR(err);
                }
            }
            i = (i+1)%num_tasks;
        ...
        for(i = 0; i < num_tasks; i++){
            mcl_transfer_wait(transfers[i]);
            mcl_transfer_free(transfers[i]);
        }
